...
function startBookingByDate() {
    showStep('step-date');
    // Show practitioner selector first for availability filtering
    showPractitionerSelectForDate();
...
// Practitioner Availability - Mock data for each practitioner
const practitionerAvailability = {
    'Melanie': {
        'Monday': ['09:00', '10:00', '11:00', '14:00', '15:00', '16:00'],
        'Tuesday': ['09:00', '10:30', '11:30', '14:00', '15:30'],
        'Wednesday': ['10:00', '11:00', '13:00', '14:00', '15:00', '16:00'],
        'Thursday': ['09:00', '10:00', '11:00', '14:00', '15:00'],
        'Friday': ['09:30', '10:30', '14:00', '15:00', '16:00', '17:00'],
    ...
    'Nadine': {
        ...
        'Friday': ['10:00', '14:00', '15:00', '16:00'],
        'Saturday': ['10:00', '14:00']
    },
    'Martina': {
        'Monday': ['14:00', '15:00', '16:00'],
...
// Practitioner selection for date-based booking
window.selectedPractitionerForDate = null;

function showPractitionerSelectForDate() {
    const stepDate = document.getElementById('step-date');
    if (!stepDate) return;

    let selector = document.getElementById('practitionerDateSelector');
    if (!selector) {
        selector = document.createElement('div');
        selector.id = 'practitionerDateSelector';
        selector.style.marginBottom = '25px';
        stepDate.insertBefore(selector, stepDate.querySelector('#calendar-container'));
    }

    let html = `
        <div style="margin-bottom: 20px;">
            <h3 style="font-size: 16px; font-weight: 600; margin-bottom: 12px;">Select Practitioner (Optional - to filter availability)</h3>
            <div style="display: flex; flex-wrap: wrap; gap: 10px;">
                <button class="practitioner-date-btn" onclick="selectPractitionerForDate(null)" style="background: var(--primary-color); color: white;">
                    All Practitioners
                </button>
    `;
    ...
    practitioners.forEach(practitioner => {
        html += `
            <button class="practitioner-date-btn" onclick="selectPractitionerForDate('${practitioner.name}')">
                ${practitioner.name}
            </button>
        `;
    });
    ...
    // Mark "All Practitioners" as selected by default
    selectPractitionerForDate(null);
}

function selectPractitionerForDate(practitionerName) {
    window.selectedPractitionerForDate = practitionerName;

    // Update button styling
    document.querySelectorAll('.practitioner-date-btn').forEach(btn => {
    ...
    const buttons = document.querySelectorAll('.practitioner-date-btn');
    buttons.forEach(btn => {
        const btnText = btn.textContent.trim();
        if ((practitionerName === null && btnText === 'All Practitioners') ||
            (practitionerName && btnText === practitionerName)) {
    ...
    // Redraw calendar with availability filter
    if (window.currentCalendarContainer) {
        const tempContainer = window.currentCalendarContainer;
        const html = tempContainer.innerHTML;
        tempContainer.innerHTML = '';
        tempContainer.innerHTML = html;
        // Redraw with filtered dates
        drawCalendarWithAvailability(new Date(new Date().getFullYear(), new Date().getMonth(), 1));
    }
}

// Override calendar drawing to show availability
function drawCalendarWithAvailability(date) {
    const container = document.getElementById('calendar-container') || document.getElementById('calendar-container-2');
    if (!container) return;
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        // Check if practitioner is available on this date
        let isAvailable = true;
        if (window.selectedPractitionerForDate) {
            const practitioner = practitioners.find(p => p.name === window.selectedPractitionerForDate);
            const availability = practitionerAvailability[window.selectedPractitionerForDate];
            if (availability && !availability[dayName]?.length) {
                isAvailable = false;
            }
        }

        const isSelected = window.selectedDate && dateObj.toDateString() === window.selectedDate.toDateString();
        const unavailableClass = (!isAvailable || isPast) ? 'empty' : '';

        html += `
            <div class="calendar-day ${isSelected ? 'selected' : ''} ${unavailableClass}"
                 onclick="${(!isAvailable || isPast) ? '' : `selectCalendarDate(new Date('${dateObj.toISOString()}'))`}"
                 style="opacity: ${(!isAvailable || isPast) ? '0.5' : '1'}; cursor: ${(!isAvailable || isPast) ? 'default' : 'pointer'};">
                ${day}
            </div>
        `;
    }
    ...
    html += `</div></div>`;
    container.innerHTML = html;
    window.currentCalendarContainer = container;
}
